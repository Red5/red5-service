 ...
 * 
 * <br>
 * <i>This class is based on original code from the XINS project, by Anthony
 * Goubard (anthony.goubard@japplis.com)</i>
 * 
...
public final class ClassLoaderBuilder {
	 ...
	 * 
	 * @param parent
	 *            the parent class loader or null if you want the current
	...
	public static ClassLoader build(File path, int mode, ClassLoader parent) {
		...
		if (mode == USE_RED5_LIB) {
			...
			if (home == null || ".".equals(home)) {
				...
				home = classLocation.substring(0,
			...
			try {
				...
				if (!red5jar.exists()) {
					System.out
							.println("Red5 server jar was not found, using fallback.");
					...
					try {
						...
						while (entry != null) {
							...
							if (entryName.startsWith("WEB-INF/lib/")
									...
									&& !standardLibs.contains(entryName
											.substring(12))) {
	...
	/**
	 * Removes older versions of libraries from a given list based on their
	 * version numbers.
	 * 
	 * @param list
	 */
	private final static void scrubURLList(List<URL> list) {
		...
		for (URL top : urls) {
			...
			if (topName.startsWith("grobo") || topName.startsWith("junit")
					|| topName.startsWith("ivy")) {
				removalList.add(top);
				continue;
			}
			...
			String prefix = topName.substring(0,
					topFirstDash != -1 ? topFirstDash : 3);
			...
			for (URL check : list) {
				...
				if (topSecondDash > 0) {
					...
					if (!Character.isDigit(topSubs[2].charAt(0))
							&& !Character.isDigit(checkSubs[2].charAt(0))) {
				...
				String checkVers = checkName
						.substring(topSecondDash != -1 ? (topSecondDash + 1)
								: (topFirstDash + 1));
				...
				String topVers = topName
						.substring(topSecondDash != -1 ? (topSecondDash + 1)
								: (topFirstDash + 1));
				int topThirdDash = -1;
				String topThirdName = null;
				if (topVers.length() > 0
						&& !Character.isDigit(topVers.charAt(0))) {
				...
				if (!Character.isDigit(checkVers.charAt(0))) {
					...
					if (topThirdName == null
							|| !topThirdName.equals(checkThirdName)) {
						continue;
					}
				...
				try {
					topVersionNumber = topVersion.length == 1 ? Integer
							.valueOf(topVersion[0]) : Integer.valueOf(
							topVersion[0]
									+ topVersion[1]
									+ (topVersion.length > 2 ? topVersion[2]
											: '0')).intValue();
				...
				try {
					checkVersionNumber = checkVersion.length == 1 ? Integer
							.valueOf(checkVersion[0])
							: Integer
									.valueOf(
											checkVersion[0]
													+ checkVersion[1]
													+ (checkVersion.length > 2 ? checkVersion[2]
															: '0')).intValue();
				} catch (NumberFormatException nfe) {
